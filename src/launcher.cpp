...
Launcher::Launcher(bool firstShow, QQuickView *w)
    : QQuickView(w)
    , m_dockInterface("com.matsya.Dock",
                    "/Dock",
                    "com.matsya.Dock", QDBusConnection::sessionBus())
    ...
    , m_bottomMargin(0)
{
    ...
    if (m_dockInterface.isValid() && !m_dockInterface.lastError().isValid()) {
        ...
        QDBusServiceWatcher *watcher = new QDBusServiceWatcher("com.matsya.Dock",
...
{
    QDBusInterface iface("com.matsya.Dock",
                         "/Dock",
                         "com.matsya.Dock",
                         QDBusConnection::sessionBus());
