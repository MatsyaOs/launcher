...
LauncherModel::LauncherModel(QObject *parent)
    ...
    , m_settings("matsyaos", "launcher-applist", this)
    , m_mode(NormalMode)
    , m_firstLoad(false)
{
    // Init datas.
...
void LauncherModel::sendToDock(const QString &key)
{
    ...
    if (index != -1) {
        QDBusMessage message = QDBusMessage::createMethodCall("com.matsya.Dock",
                                                              "/Dock",
                                                              "com.matsya.Dock",
                                                               "add");
...
bool LauncherModel::launch(const QString &path)
{
    ...
    if (index != -1) {
        ...
        // matsya-screenshot needs to be processed.
        if (cmd == "matsya-screenshot") {
